...
get("player").update = function(self, character, data)
	...
	task.spawn(function()
		...
		if visible and check() then
			...
			-- Fixed name text size - now much bigger and more visible
			drawings.name.Text = ` {player.Name} `
			drawings.name.Size = math.max(math.min(math.abs(16 * scale), 20), 14)  -- Increased from 12.5 max and 10 min
			...
			drawings.healthOutline.From = Vector2.new(xPosition - 5, yPostion - 1)
			drawings.healthOutline.To = Vector2.new(xPosition - 5, yPostion + height + 1)
			drawings.health.From = Vector2.new(xPosition - 5, yPostion + height)
			drawings.health.To = Vector2.new(xPosition - 5, (yPostion + height) - ((height / 100) * healthPercent))
			
			-- Fixed health text size - bigger and more visible
			drawings.healthText.Text = ` HP {math.floor(humanoid.Health)} `
			drawings.healthText.Size = math.max(math.min(math.abs(14 * scale), 18), 12)  -- Increased from 11 max and 10 min
			...
			-- Fixed distance text size - bigger and more visible
			drawings.distance.Text = ` {math.floor(data.distance)} `
			drawings.distance.Size = math.max(math.min(math.abs(14 * scale), 18), 12)  -- Increased from 11 max and 10 min
			...
			-- Fixed weapon text size - bigger and more visible
			drawings.weapon.Text = weapon
			drawings.weapon.Size = math.max(math.min(math.abs(12 * scale), 16), 11)  -- Slightly smaller
